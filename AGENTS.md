...
When making substantial product changes, feature work, provider changes, UI changes, release/install changes, or user-visible behavior fixes, do not silently finish without considering the in-app version history.

At the end of a meaningful development session, ask the user whether today's development task is finished. Use a concise question such as:

> 今天这轮开发任务是否已经结束？如果结束，我会 bump 版本号并更新“版本更新”历史记录。

If the user confirms the session is finished:

1. Increment the version, including patch/minor versions for small releases.
   - Update `MARKETING_VERSION` and `CURRENT_PROJECT_VERSION` in `project.yml`.
   - Run `xcodegen generate` after changing `project.yml`.
   - Ensure `OpenTypeless/Resources/Info.plist` continues to use `$(MARKETING_VERSION)` and `$(CURRENT_PROJECT_VERSION)`.
2. Add a new entry at the top of `OpenTypeless/Models/VersionHistory.swift`.
   - Every shipped/tested version must have an entry, even small patch versions.
   - Include version, build number, date, a short title, and concise user-facing change bullets.
   - Mention user-visible testing cues such as new providers, connection/error states, paste behavior fixes, app icon changes, install changes, or settings changes.
3. Verify the app version shown in the app matches the bundle version.
   - Build with the workspace.
   - Check the built or installed app's `CFBundleShortVersionString` and `CFBundleVersion` when practical.
4. If installing for local testing, replace `/Applications/OpenTypeless.app` only when the user asks or when that is part of the requested workflow.

If the user says the session is not finished, do not bump the version yet. Keep working normally and ask again after the next substantial milestone.
